      ...
      <div className="relative flex bg-gradient-to-b from-primary-80 via-20% via-white  to-secondary-80  w-[1100px] min-h-[450px] rounded-xl drop-shadow-2xl shadow-2xl shadow-black/60 flex-col p-10 items-center text-primary-0" onClick={(e) => e.stopPropagation()} >
        ...
        <form onSubmit={handleSubmit} action="" className="grid grid-cols-2 w-full gap-x-20 relative px-16">
          <div className="flex flex-col gap-10 items-center">
            ...
            <InputCreateModal editable={editable} value={complex?.apertureDate} htmlFor="apertureDate" type="date" handleChange={handleChange} label={t("create_complex.aperture_date")} inputClassName="px-6 py-1 bg-primary-20 text-white rounded-full shadow-inner shadow-black border-2 border-primary-50 custom-date-input" />
          </div>
          <div className="col-span-1 col-start-2 gap-4 mb-32 grid grid-cols-2 ">
            <InputCreateModal editable={editable} value={complex?.title} htmlFor="title" type="text" handleChange={handleChange} label={t("create_complex.title")} containerClassName="w-full flex flex-col gap-1 col-span-2" />
            <InputCreateModal editable={editable} value={complex?.cuit} htmlFor="cuit" type="text" handleChange={handleChange} label={t("create_complex.SSN")} />
            <InputCreateModal editable={editable} value={complex?.address?.street} htmlFor="street" type="text" handleChange={handleChange} label={t("create_complex.street")} />
            <InputCreateModal editable={editable} value={complex?.address?.postalCode} htmlFor="postalCode" type="text" handleChange={handleChange} label={t("create_complex.postal_code")} />
            <InputCreateModal editable={editable} value={complex?.address?.city} htmlFor="city" type="text" handleChange={handleChange} label={t("create_complex.city")} />
            <InputCreateModal editable={editable} value={complex?.phoneNumber} htmlFor="phoneNumber" type="text" handleChange={handleChange} label={t("create_complex.phone_number")} />
          </div>
